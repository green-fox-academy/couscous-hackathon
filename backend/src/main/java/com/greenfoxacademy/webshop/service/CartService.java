...
public class CartService {
  ...
  public void addItemToCart (CartItemRequestDTO request, String cartId) throws ItemNotFoundException {
    Cart cart = cartRepository.findById(cartId).orElse(cartRepository.save(new Cart(cartId,new Timestamp(System.currentTimeMillis()),null)));
    Optional<CartAmount> optionalCartAmount = cartAmountRepository.findCartAmountByItem_IdAndCart_Id(request.getItemId(),cartId);
